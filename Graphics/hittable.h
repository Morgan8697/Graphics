...
public:
	translate(shared_ptr<hittable> hittable_object, const vec3& offset) : hittable_object(hittable_object), offset(offset)
	{
		bbox = hittable_object->bounding_box() + offset;
	}
	...
	bool hit(const ray& r, interval ray_t, hit_record& rec) const override {
		...
		// Determine whether an intersection exists along the offset ray (if so, where)
		if (!hittable_object->hit(offset_r, ray_t, rec))
			return false;
...
private:
	shared_ptr<hittable> hittable_object;
	vec3 offset;
	aabb bbox;
};
...
public:
	rotate_y(shared_ptr<hittable> object, double angle) : hittable_object(object) {
	...
	bool hit(const ray& r, interval ray_t, hit_record& rec) const override {
		...
		point3 origin = point3((cos_theta * r.origin().x()) - (sin_theta * r.origin().z()), r.origin().y(), (sin_theta * r.origin().x()) + (cos_theta * r.origin().z()));
		point3 direction = point3((cos_theta * r.direction().x()) - (sin_theta * r.direction().z()), r.direction().y(), (sin_theta * r.direction().x()) + (cos_theta * r.direction().z()));

		ray rotated_r(origin, direction, r.time());

		// Determine whether an intersection exists in object space (and if so, where).

		if (!hittable_object->hit(rotated_r, ray_t, rec))
			return false;

		// Transform the intersection from object space back to world space.
		rec.p = point3((cos_theta * rec.p.x()) + (sin_theta * rec.p.z()), rec.p.y(), (-sin_theta * rec.p.x()) + (cos_theta * rec.p.z()));
